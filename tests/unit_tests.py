...
def one_element_integration_test(m, cam):
    ...
    m.Plot()
    plt.plot(m.pgx,m.pgy,'k.')
    plt.title('DICIntegration (rectangle rule)')
    ...
    print("MASS Gauss vs Rect %2f %%" % (np.linalg.norm(M0 - M1) / np.linalg.norm(M0) * 100, ) )
    print("STIFFNESS Gauss vs Rect %2f %%" % (np.linalg.norm(K0 - K1) / np.linalg.norm(K0) * 100, ) )
    print("TIKO Gauss vs Rect %2f %%" % (np.linalg.norm(L0 - L1) / np.linalg.norm(L0) * 100, ) )
