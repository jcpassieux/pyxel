...
def Get1dBasisFunctionsAtPts(degree, knotVect, pts, der):
    ...
    for i in range(nb_pts_values):
        ...
        for j in range(degree+1):
            ...
            k+=1 
    for d in range(der+1):
        operators[d] = sps.csc_matrix(( values[d], (indexI,indexJ)), shape = (nb_pts_values,n) )
    return operators 
...
def Get3dBasisFunctionsAtPts(x,y,z,Xi,Eta,Zeta,p,q,r):
    nb_x_values = len(x);
    nbf_pt      = (p+1)*(q+1)*(r+1)  # Number of basis functions that support a point 
    ...
    for up in range(nb_x_values):
        ...
        indexJ[  index + up*nbf_pt ] = (spanx-p+index_i) + (spany-q+index_j)*nbf_xi + (spanz-r+index_k)*nbf_xi*nbf_eta
        ...
        #             ll = ll +1
    phi     = sps.csc_matrix(( valuesN,    (indexI,indexJ)), shape = (nb_x_values,nbf) )
    return phi  
